...
<p align="center">
  ...
  <br><sub>网关总览：集中完成启动、停止、重启、健康检查、地址复制和最近操作查看。</sub>
</p>
...
「模型」页会按真实模型 ID 汇总配置。可以对某个模型单独开启分流，再分别决定哪些上游参与；关闭线路不需要删除密钥或配置。

<p align="center">
  <img src="docs/assets/studio-model-routing-v1.4.0.png" width="920" alt="按模型管理多账号分流">
</p>

### 实时分流预览

请求实际经过某条线路后，带电流效果的连接会常驻在图中。模型选中了哪家平台一眼可见，同时只保留经过隐私收敛的聚合信息。
